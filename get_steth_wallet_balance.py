...
def get_steth_balance():
    ...
    steth_balance_smallest_decimal = get_token_balance(etherscan_api_key, contract_address, wallet_address)
    steth_balance_normal_decimal = smallest_decimal_to_normal(int(steth_balance_smallest_decimal), decimal_places)
    steth_balance_normal_decimal_truncated = truncate_to_range(steth_balance_normal_decimal, min_value=1e-8, max_value=1000000000000)

    if steth_balance_normal_decimal_truncated is not None:
        print(f"STETH残高: {steth_balance_normal_decimal_truncated}")
        jpy_amount = convert_to_jpy(coinmarketcap_api_key, steth_balance_normal_decimal_truncated, "STETH")
